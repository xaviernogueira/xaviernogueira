...
⏳ [`Xarray-DataAccessor`](https://github.com/LimnoTech/Xarray-DataAccessor) - Working for [LimnoTech](https://www.limno.com/) I was tasked with creating a Python library that faciliates rapid read-in of remote climate, topographic, land cover, and soils data into standardized `xarray.Dataset` objects. `dask.distributed` is used to multi-thread I/O from both S3 cloud buckets and APIs. One advantage of this implementation is allowing the user to track progress via the Dask Dashboard, but more importantly, data read-in can be scaled-up arbitrarily to fully utilize available computational resources. The library was built using a [factory architecture pattern](https://en.wikipedia.org/wiki/Factory_method_pattern) in order to both create a unified "user interface" that abstracts dataset specific details as well as to encourage open-source contributions/plugins. `Xarray-DataAccessor` also enables a variety of data transformations, point based sampling, and datset "alignment" to match a variety of hydrologic model input specifications. This library is still a work in progress...more features are planned!
